...
#
# This configuration assumes the presence of an internal routing 
# instance. For more details, see the associated diagram. Here, we assume  
# the internal routing instance is named "YOUR_RI".
#
set routing-instances YOUR_RI instance-type virtual-router
</xsl:text>
...
set security zones security-zone trust interfaces </xsl:text><xsl:value-of select="$tunintf"/>
      <xsl:text>
set routing-instances YOUR_RI interface </xsl:text><xsl:value-of select="$tunintf"/>
      <xsl:text>
      
# To establish connectivity between your internal network and the VPC, you
# must have an interface facing your internal network in the YOUR_RI
# routing instance.
# This is done using configuration like that shown below.
# set routing-instances YOUR_RI interface ge-0/0/1.0
...
set routing-instances YOUR_RI protocols bgp group ebgp type external

</xsl:text>

      <xsl:variable name="base" select="concat('set routing-instances YOUR_RI protocols bgp group ebgp neighbor ', $peer)"/>
      ...
      <xsl:value-of select="./customer_gateway/bgp/asn" /><xsl:text>
#
</xsl:text>
...
# set security ike traceoptions flag all
   ...
   <xsl:template match="ipsec_tunnel/ipsec">
      ...
      <xsl:variable name="ipsecpol" select="concat('ipsec-pol-',$id)" />
...
set security ipsec policy </xsl:text><xsl:value-of select="$ipsecpol" />
      ...
      <xsl:text>
set security ipsec policy </xsl:text><xsl:value-of select="$ipsecpol" />
      <xsl:text> proposals ipsec-prop-</xsl:text>
